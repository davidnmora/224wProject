...
#        egonets[5].append(GenRndGnm(PUNGraph, egoI.GetNodes(), egoI.GetEdges()*2, False))
        egonets[6].append(GenSmallWorld(egoI.GetNodes(), egoI.GetEdges()/egoI.GetNodes(), .5))
...
for j in egos:
    ...
    for i in j:
        ...
        #Components is every circle in ego network
        #cnCom is all nodes in a single circle
        for cnCom in Components:
